...
$plugin_active = in_array( 'palm-reading/mystic-palm-reading.php', $active_plugins );
WP_CLI::line( 'Plugin Active: ' . ( $plugin_active ? WP_CLI::colorize( '%G✓ YES%n' ) : WP_CLI::colorize( '%R✗ NO%n' ) ) );

if ( ! $plugin_active ) {
    WP_CLI::error( 'Plugin is not active. Activate it first with: wp plugin activate palm-reading' );
}
...
WP_CLI::line( '---' );
$constants = array( 'SM_VERSION', 'SM_PLUGIN_DIR', 'SM_PLUGIN_URL', 'SM_PLUGIN_FILE' );
...
WP_CLI::line( '---' );
$log_file = SM_PLUGIN_DIR . '/debug.log';
...
$shortcode_exists = isset( $shortcode_tags['soulmirror_palm_reading'] );
WP_CLI::line( '[soulmirror_palm_reading]: ' . ( $shortcode_exists ? WP_CLI::colorize( '%G✓ Registered%n' ) : WP_CLI::colorize( '%R✗ Not Registered%n' ) ) );

// 9. Check asset files
WP_CLI::line( '' );
WP_CLI::line( '9. FRONTEND ASSETS' );
WP_CLI::line( '---' );
$js_file = SM_PLUGIN_DIR . '/assets/js/script.js';
$css_file = SM_PLUGIN_DIR . '/assets/css/styles.css';
